...
RSpec.describe 'Testing Category#index view', type: :feature do
  ...
  it 'renders the category list with correct information' do
    ...
    expect(page).to have_selector('header h1', text: 'CATEGORIES')
    expect(page).to have_selector('header i.fa-solid.fa-magnifying-glass')
    ...
    @categories.each do |category|
      ...
      within(".category-link[href='#{user_category_expenses_path(@user, category)}']") do
        ...
        expect(page).to have_selector('.category p.name', text: category.name)
        expect(page).to have_selector('.category p.expense-date', text: category.created_at.strftime('%d %b %Y'))
